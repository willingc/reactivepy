...
from typing import Union, Any, Dict, FrozenSet, List, Tuple
...
class ExecutionUnitInfo:
    ...
    def __init__(self, code_obj: CodeObject, pinning_cell: Union[None, str] = None):
...
class ExecBlock:
    ...
    def __init__(
        self,
        request: RequestInfo,
        current_exec_unit: ExecutionUnitInfo,
        descendant_exec_units: List[ExecutionUnitInfo],
        async_gen_obj=None,
    ):
...
class ReactivePythonKernel(Kernel):
    ...
    def __init__(self, **kwargs):
        ...
        self.KernelTB = ultratb.AutoFormattedTB(
            mode='Plain', color_scheme='LightBG', tb_offset=1, debugger_cls=None
        )
    ...
    async def _run_single_async_iter_step(self, item, exec_unit):
        ...
        request_id, async_gen_obj, insert_id = self._registered_generators[target_id]
        async with self._inner_state_lock:
            if request_id != item.request.msg_id and insert_id < item.id:
                ...
                self._registered_generators[target_id] = (
                    item.request.msg_id,
                    item.async_gen_obj,
                    item.id,
                )
    ...
    async def _first_async_iter_step(
        self,
        item: ExecBlock,
        ...
        async with self._inner_state_lock:
            ...
            self._registered_generators[exec_result.target_id] = (
                item.request.msg_id,
                exec_result.output,
                item.id,
            )
    ...
    def _update_existing_exec_unit(self, code_obj, cell_id):
        ...
        # add new edges
        exec_unit = self._exec_unit_container.get_by_display_id(code_obj.display_id)
    ...
    async def _inner_execute_request_callback(self, stream, ident, parent):
        ...
        if not request.silent:
            ...
            self._publish_execute_input(request.code, parent, request.execution_count)
        ...
        response_meta = self.finish_metadata(parent, response_meta, reply_content)
        ...
        if (
            not request.silent
            and reply_msg['content']['status'] == u'error'
            and stop_on_error
        ):
            self._abort_queues()
    ...
    def execute_request(self, stream, ident, parent):
        ...
        self._eventloop.spawn_callback(
            self._inner_execute_request_callback, stream, ident, parent
        )
    ...
    def _update_kernel_state(self, code_obj, cell_id, deleted_cell_ids):
        ...
        try:
            ...
            if self._exec_unit_container.contains_display_id(code_obj.display_id):
                current_exec_unit = self._update_existing_exec_unit(code_obj, cell_id)
            else:
                current_exec_unit = self._create_new_exec_unit(code_obj, cell_id)
    ...
    def _output_exec_results(
        ...
        if len(stdout) > 0:
            self.session.send(
                ...
                message_mode,
                content={
                    'data': {'text/plain': stdout},
                    'metadata': {},
                    'transient': {'display_id': exec_unit.stdout_display_id},
                },
        ...
        if exec_result.output is not None:
            self.session.send(
                ...
                content={
                    'data': data,
                    'metadata': md,
                    'transient': {'display_id': exec_unit.display_id},
                },
                parent=request.parent,
            )
        ...
        if len(stderr) > 0:
            self.session.send(
                self.iopub_socket,
                'stream',
                content={'name': 'stderr', 'text': stderr},
                parent=request.parent,
            )
    ...
    async def do_execute(self, request: RequestInfo):
        try:
            ...
            code_obj = CodeObject(
                request.code, self._key, self._execution_ctx.manager_ns
            )

            # 2. Extract metadata (both are optional)
            cell_id = (
                request.metadata['cellId'] if 'cellId' in request.metadata else None
            )
            deleted_cell_ids = (
                request.metadata['deletedCells']
                if 'deletedCells' in request.metadata
                else None
            )
            ...
            async with self._inner_state_lock:
                ...
                descendants = self._dep_tracker.get_descendants(code_obj.display_id)
                descendant_exec_units = list(
                    map(
                        lambda display_id: self._exec_unit_container.get_by_display_id(
                            display_id
            ...
            block = ExecBlock(request, current_exec_unit, descendant_exec_units)
        ...
        return {'status': 'ok', 'execution_count': request.execution_count}


async def anext(*args):
    if len(args) < 1:
        raise TypeError(f"anext expected at least 1 arguments, got {len(args)}")
